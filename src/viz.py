...
from src.prep import get_data, random_color


def add_nodes_to_graph(graph: object, nodes: pd.DataFrame):
    for _, char in nodes.iterrows():
        graph.add_node(
            char.name,
            size=char.Size,
            name=char.name,
...
def add_links_to_graph(
    graph: object, links: pd.DataFrame, nodes: pd.DataFrame
):
...
def create_node_trace(node_cluster: pd.DataFrame, name: str) -> object:
    ...
    return go.Scatter3d(
        ...
        showlegend=True,
        customdata=np.stack(
            [node_cluster.description, node_cluster.group], axis=1
        ),
        hovertemplate=(
            "%{text}"
...
def create_edge_trace(
    ...
    return go.Scatter3d(
        ...
        line=dict(width=1, color=edge_cluster.rel_color),
        visible=True,
        hoverinfo="text",
        mode="lines",
    )
...
def create_plot(links, nodes, graph):
    ...
    pos_ = nx.spring_layout(
        graph, dim=3, k=2, iterations=100, threshold=0.00001, seed=93
    )  # get coordinates for nodes in spring layoyt

    placed_nodes = pd.merge(
        nodes,
        pd.DataFrame(
            zip(*get_node_coordinates(graph, pos_)), columns=["Xn", "Yn", "Zn"]
        ),
        left_index=True,
        right_index=True,
    )

    node_trace = create_node_trace(placed_nodes, name="nodes")

    edge_trace = create_edge_trace(
        *get_link_coordinates(graph, placed_nodes), links, name="edges"
    )
